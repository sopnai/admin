...
const BookingView = () => {
    ...
    const refundCalculation = (item) => {
        ...
        const halfpaid = total > 0 ? total / 2 : 0;
        if ((bookingTotalAmount - bookingAmountPaid) != 0) {
            if (item?.cancelbyclient === 1 && item?.status === "cancelled") {
                return "$ " + (halfpaid / 2).toFixed(2);
            }
            if (item?.status === "cancelled") {
                return "$ " + (halfpaid).toFixed(2);
            }
        } else {
            if (item?.cancelbyclient === 1 && item?.status === "cancelled") {
                                                ...
                                                {bookings?.bookingItems?.map((item, index) => (
                                                    <table key={item.id || index} className="table table-bordered mb-4">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                <td colSpan={5}>{item.artistDetails?.address || "-"}</td>
                                                                <td>
                                                                    <Badge className={`badge-size ${getStatusClass(item.status)}`}>
                                                                        {item.status}
                                                                    </Badge>

                                                                    {item?.cancelbyclient !== 1 && item?.status === "cancelled" ? (
                                                                        <p>[Cancelled by Artist]</p>
                                                                    ) : null}
                                                                </td>
                                                                <td>
                                                                    {refundCalculation(item)}
                                                                </td>
                                                            </tr>
